...
class ReviewController extends ChangeNotifier {
  ...
  void pickImage(bool isRemove, {bool fromReview = false}) async {
    if(isRemove) {
      ...
      _imageFile = await ImagePicker().pickImage(source: ImageSource.gallery, imageQuality: 100);
      if (_imageFile != null) {
        double imageSizeInMB =  await ImageSize.getImageSizeFromXFile(_imageFile!);

        ConfigModel? configModel = Provider.of<SplashController>(Get.context!, listen: false).configModel;
        if(imageSizeInMB > (configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)) {
