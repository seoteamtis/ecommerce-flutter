        ...
        Consumer<BannerController>(
          builder: (context, bannerProvider, child) {
            ...
            return bannerProvider.mainBannerList != null
              ...
              children: [
                SizedBox(
                  // Task (b): Height ko width ke hisaab se adjust kiya (aap ise 1.2 ya 1.5 bhi kar sakte hain agar image lambi chahiye)
                  height: width * 1.0,
                  width: width,
                  child: CarouselSlider.builder(
                    options: CarouselOptions(
                      aspectRatio: 1/1,      // (b) Full width look ke liye 1:1 ya 4/5 rakhein
                      viewportFraction: 1.0, // (b) Sabse important: Isse image kine tak touch hogi
                      autoPlay: true,        // (a) Static slider behavior
                      enlargeCenterPage: false, // Enlarge false kiya taaki side gaps na aayein
                      disableCenter: true,
                      onPageChanged: (index, reason) {
                        Provider.of<BannerController>(context, listen: false).setCurrentIndex(index);
                      },
                    ...
                    itemBuilder: (context, index, _) {
                      return InkWell(
                        onTap: () {
                          if (bannerProvider.mainBannerList![index].resourceId != null || bannerProvider.mainBannerList![index].url != null) {
                            bannerProvider.clickBannerRedirect(
                              context,
                              bannerProvider.mainBannerList![index].resourceId,
                              bannerProvider.mainBannerList![index].resourceType == 'product' ? bannerProvider.mainBannerList![index].product : null,
                              bannerProvider.mainBannerList![index].resourceType,
                              url: bannerProvider.mainBannerList![index].url,
                            );
                          }
                        ...
                        child: Container(
                          ...
                          // Task (b): BorderRadius hata diya hai taaki corners screen se chipke rahein (Full Width)
                          child: CustomImageWidget(
                            image: '${bannerProvider.mainBannerList?[index].photoFullUrl?.path}',
                            fit: BoxFit.cover, // Image ko poore area mein fill karne ke liye
                            width: width,
                          ),
                        ),
                      );
                ...
                // Indicator Dots (Agar client ki image jaisa clean chahiye toh ise hata bhi sakte hain)
                if (bannerProvider.mainBannerList != null && bannerProvider.mainBannerList!.isNotEmpty)
                  Positioned(
                    bottom: 15,
                    left: 0,
                    ...
                    child: Row(
                      ...
                      children: bannerProvider.mainBannerList!.map((banner) {
                        int index = bannerProvider.mainBannerList!.indexOf(banner);
                        return Container(
                          width: index == bannerProvider.currentIndex ? 18 : 7,
                          height: 7,
