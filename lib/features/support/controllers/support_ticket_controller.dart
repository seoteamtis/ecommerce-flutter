...
class SupportTicketController extends ChangeNotifier {
  ...
  void pickMultipleImage(bool isRemove,{int? index}) async {
    if(isRemove) {
      ...
      _pickedImageFiles = await ImagePicker().pickMultiImage(imageQuality: 100);
      for(XFile image in _pickedImageFiles){
        double imageFileSize =  await ImageSize.getImageSizeFromXFile(image);
        ConfigModel? configModel = Provider.of<SplashController>(Get.context!, listen: false).configModel;

        if(imageFileSize > (configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)) {
          showCustomSnackBarWidget(getTranslated('${getTranslated('maximum_image_size', Get.context!)} ${(configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)}MB', Get.context!), Get.context!, snackBarType: SnackBarType.warning);
        } else{
